function [straight_pos_traj, straight_vel_traj] = fun_generate_straight_traj(start_position, move_len_mm, move_speed_mmps, direction, dt)
% 직선 구간에 대한 속도 프로파일 생성
%
% 입력
%   start_position      : 직선 경로 시작 위치 좌표
%   move_len_mm         : 로봇 목표 이동 거리
%   move_speed_mmps    : 로봇 이동 속도
%   direction           : 로봇 이동 방향
%   dt                  : 제어 주기
%
% 출력
...
%   straight_vel_traj   : 직선 구간 속도 경로 (Nx6 매트릭스) - [vx,vy,vz,wx,wy,wz]
    ...
    % 총 이동 시간 계산
    totalTime = move_len_mm / move_speed_mmps;
    
    % 시간 벡터 생성
    traj_t = (0:dt:totalTime)';

    traj_t_accel = totalTime * 0.3;                             % 가속에 걸리는 시간 (s)
    ...
    speedProfile(1:length(t_accel)) = accel_shape * move_speed_mmps;
    ...
    speedProfile(start_const_idx:end_const_idx) = move_speed_mmps;
    ...
    speedProfile(start_decel_idx:end) = decel_shape * move_speed_mmps;
    ...
    scaling_factor = move_len_mm / distanceProfile_raw(end);
    ...
    pos_traj_xyz = start_position(1:3) + distanceProfile .* unit_direction(1:3);

    % 3. 로봇 경로 벡터 정보 결합
    % 경로점 개수만큼 시작 자세를 복제합니다.
    num_points = length(speedProfile);
    pos_traj_orient = repmat(start_position(4:6), num_points, 1);
