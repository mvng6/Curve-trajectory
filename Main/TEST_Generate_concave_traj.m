%% 오목한 테스트 경로 생성 스크립트
% 설명: 아래로 오목한 완만한 곡률을 가진 3D 경로와 법선 벡터를 생성하여
%       'test_concave_traj.csv' 파일로 저장합니다.
% 생성 경로: X축을 따라 진행하며 Z축 방향으로 부드러운 골짜기 모양을 가집니다.
...
amplitude = 30.0;       % 골짜기의 깊이 (Z축 방향, mm). 이 값이 클수록 곡률이 커집니다.
num_points = 100;      % 경로를 구성할 원본 점의 개수

fprintf('  - 경로 길이: %.1f mm\n', path_length);
fprintf('  - 골짜기 깊이: %.1f mm\n', amplitude);
fprintf('  - 점 개수: %d 개\n\n', num_points);
...
% Z축: 코사인 함수를 이용하여 부드러운 골짜기 모양 생성
...
figure('Name', 'Concave Test Trajectory', 'NumberTitle', 'off');
