function [curve_pos_traj, curve_vel_traj] = fun_generate_curve_traj(start_position, move_len_mm, move_speed_degs, direction, ...
...
%   move_len_mm             : 원하는 곡면 이동 거리
%   move_speed_degs         : 목표 로봇 이동 속도
%   direction               : 곡면 구동 방향
%   interp_position_spline  : spline으로 보간 된 위치 좌표 행렬
%   interp_normals_slerp    : SLERP로 보간된 법선 벡터 행렬
%   omega_slerp             : SLERP로 보간된 법선 벡터의 각속도 행렬
...
%   curve_vel_traj          : 곡선 구간 속도 경로 (Nx6 매트릭스) - [vx,vy,vz,wx,wy,wz]
    ...
    if (home_flag == 0)
        ...
        distances_from_start = vecnorm(interp_position_spline(:,1:3) - start_position(:,1:3), 2, 2);
        ...
        % (3) 목표 누적 거리 계산
        target_dist_mm = start_dist_mm + move_len_mm;
        ...
        curve_pos_traj = [interp_position_spline(start_index:final_index, :), interp_normals_slerp(start_index:final_index,:)];
        ...
        %% 속도 프로파일 생성
        robot_speed = max(omega_slerp(start_index:final_index));
        speedProfile = zeros(num_path_points, 1);
        
        % 가감속 구간을 전체 경로점의 15%씩 할당 (S-커브는 약간 더 여유있게)
        num_accel_points = round(num_path_points * 0.3);
        num_decel_points = round(num_path_points * 0.3);
        
        % 가감속 포인트가 너무 적을 경우를 대비한 최소값 설정
        ...
        %   (A) 가속 구간 생성 (S-커브)
        accel_shape = move_speed_degs * 0.5 * (1 - cos(pi * (1:num_accel_points)' / num_accel_points));
        ...
        const_phase = ones(num_const_points, 1) * move_speed_degs;
        speedProfile(start_const_idx:end_const_idx) = const_phase;
        
        %   (C) 감속 구간 생성 (S-커브)
        start_decel_idx = end_const_idx + 1;
        decel_shape = move_speed_degs * 0.5 * (1 + cos(pi * (1:num_decel_points)' / num_decel_points));
    ...
    elseif (home_flag == 1)
        ...
        % 입력 인덱스의 유효성 검사
        if start_index < 1 || end_index > size(interp_position_spline, 1) || start_index <= end_index
            error('유효하지 않은 인덱스 범위입니다. start_index는 end_index보다 커야 합니다.');
        end
        
        % (2) 최종 곡선 구간의 위치/자세 경로(Nx6) 추출
        curve_pos_traj = [interp_position_spline(start_index:-1:end_index,:), interp_normals_slerp(start_index:-1:end_index,:)];
        curve_pos_traj(:,2) = curve_pos_traj(:,2) + ones(length(curve_pos_traj),1)*offset;
        ...
        %% 속도 프로파일 생성
        robot_speed = max(omega_slerp(start_index:-1:end_index));      
        speedProfile = zeros(num_path_points, 1);
