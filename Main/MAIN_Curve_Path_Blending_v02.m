%% Info
% 250826 원본 경로에 노이즈 제거 후속도 경로 생성
%   => 원본 경로에 대하여 노이즈를 제거한 뒤 블렌딩 및 리샘플링 후 각속도를 계산
%   => 원본 경로 -> 스무딩 -> 블렌딩 & 리샘플링 -> 최종 경로 계산
%
% --- Result ---
%   => 
...
smoothed_data = fun_smoothPathSavitzkyGolay(original_data, poly_order, window_size);
fprintf('  -> 스무딩 완료.\n');

% 시각화
figure('Name', '스무딩 결과 비교', 'NumberTitle', 'off');
hold on; grid on; axis equal;
plot3(original_data(:,1), original_data(:,2), original_data(:,3), 'Color', [0.5 0.5 0.8], 'DisplayName', 'Original Traj');
plot3(smoothed_data(:,1), smoothed_data(:,2), smoothed_data(:,3), 'r-', 'LineWidth', 2, 'DisplayName', 'Smoothed Traj');
...
title('Original Traj vs. Smoothed Traj');
xlabel('X (mm)'); ylabel('Y (mm)'); zlabel('Z (mm)');
legend; view(3);
fprintf('  -> 스무딩 한 경로 시각화 완료.\n\n');
...
if (blend_flag == true)
    fprintf('3. 스무딩 된 경로에 블렌딩 처리를 적용합니다...\n');
    ...
    % 3-1. 경로에서 코너점 찾기
    corner_indices = fun_find_path_corners(smoothed_data(:,1:3), config.detection);

    % 3-2. 찾은 코너점에 블렌딩 적용하기
    [new_P, new_N] = fun_apply_path_blending_normals(smoothed_data(:,1:3), smoothed_data(:,4:6), corner_indices, config.blending.distance);
    fprintf('  -> 경로 블렌딩 완료. (블렌딩 설정 거리: %dmm)\n\n',config.blending.distance);
else
    fprintf('3. 스무딩 된 경로에 블렌딩 처리를 적용하지 않습니다...\n');
    new_P = smoothed_data(:,1:3);
    new_N = smoothed_data(:,4:6);
end
...
% 마지막 점은 이전 값으로 채움
final_trajectory.vel_tool(end,:) = final_trajectory.vel_tool(end-1,:);      % mm/s
final_trajectory.omega_tool(end,:) = final_trajectory.omega_tool(end-1,:);  % rad/s
final_trajectory.omega_tool = rad2deg(final_trajectory.omega_tool);         % deg/s
...
plot3(P(:,1), P(:,2), P(:,3), 'k:', 'LineWidth', 1.5, 'DisplayName', 'Original Traj');
plot3(final_trajectory.position(:,1), final_trajectory.position(:,2), final_trajectory.position(:,3), 'r-', 'LineWidth', 2, 'DisplayName', 'Final Traj');
