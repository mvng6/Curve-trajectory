%% Info
%
% 250825 원본 경로에 대한 속도 경로 생성
%   => 원본 경로에 대하여 블렌딩 및 리샘플링 후 각속도를 계산
%   => 리샘플링 과정에서 원하는 제어 주기와 선속도 값을 지정 (등속도 구동)
%
% --- Result ---
%   => 각속도가 빠른 주기의 사인파 형태로 출렁이는 결과가 도출됨
%   => 기존 경로에서 곡률 부분에 매우 미세한 노이즈가 원인으로 판단
%   => 원본 경로에 해당 노이즈를 제거한 뒤 앞선 경로 생성 과정을 적용하는 것이 필요
%   => MAIN_Curve_Path_Blending_v02 코드에서 노이즈 제거를 적용할 예정
