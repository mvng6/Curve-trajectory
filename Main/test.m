...
[interp_time_s, interp_position_spline, interp_normals_slerp, interp_dist_mm] = fun_resample_interp(traj_file_name, robot_speed_mmps, dt);
...
% 직선 구간
% 첫 번째 경로 지점 생성
traj_start = [interp_position_spline(1,:), interp_normals_slerp(1,:)];
direction = [0, 1, 0, 0, 0, 0];
[straight_pos_traj, straight_vel_traj] = fun_generate_straight_traj(traj_start, ...
    straight_move_len_mm, direction, robot_speed_mmps, dt);
...
[curve_pos_traj, curve_vel_traj] = fun_generate_curve_traj(start_position, ...
    interp_position_spline, interp_normals_slerp, ...
    omega_slerp, interp_dist_mm, direction, curve_move_len_mm, offset, home_flag);
...
% 직선 구간
% 세 번째 경로 지점 생성
traj_start = traj_pos_profile(end,:);
direction = [0, -1, 0, 0, 0, 0];
[straight_pos_traj, straight_vel_traj] = fun_generate_straight_traj(traj_start, ...
    straight_move_len_mm, direction, robot_speed_mmps, dt);
...
%%
figure;
plot(traj_vel_profile(:,2))
hold on;
plot(traj_vel_profile(:,5)*335.0*(pi/180))